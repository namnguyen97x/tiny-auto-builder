...
# Function to auto-download IRST driver
...
function Add-DriverToImage {
    ...
    if ((-not $DriverPath -or -not (Test-Path $DriverPath)) -and $AutoDownload) {
        Write-Host "Auto-downloading IRST driver..."
...
if ($IrstDriverPath -or $AutoDownloadIrst) {
    Add-DriverToImage -MountPath $scratchDir -DriverPath $IrstDriverPath -ImageName "install.wim" -AutoDownload:$AutoDownloadIrst -DownloadUrl $IrstDriverUrl
}
...
Write-Host "Mounting Windows Setup image (index 2) to modify registry..."
& 'dism' '/English' '/mount-image' "/imagefile:$bootWimPath" '/index:2' "/mountdir:$scratchDir"
reg load HKLM\zDEFAULT $mainOSDrive\scratchdir\Windows\System32\config\default | Out-Null
reg load HKLM\zNTUSER $mainOSDrive\scratchdir\Users\Default\ntuser.dat | Out-Null
reg load HKLM\zSOFTWARE $mainOSDrive\scratchdir\Windows\System32\config\SOFTWARE | Out-Null
reg load HKLM\zSYSTEM $mainOSDrive\scratchdir\Windows\System32\config\SYSTEM | Out-Null
...
if ($IrstDriverPath -or $AutoDownloadIrst) {
    Add-DriverToImage -MountPath $scratchDir -DriverPath $IrstDriverPath -ImageName "boot.wim (Windows Setup)" -AutoDownload:$AutoDownloadIrst -DownloadUrl $IrstDriverUrl
}
